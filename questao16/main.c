...
int main() {
  ...
  float* nums1 = malloc(n*sizeof(float));
  for (int i = 0; i < n; i++) {
    nums1[i] = randomf();
  }

  float* nums2 = malloc(n*sizeof(float));
  memcpy(nums2, nums1, n*sizeof(float));

  clock_t begin;
  clock_t end;
  begin = clock();
  bubbleSort(nums1, n, cmp);
  end = clock();
  printf("tempo de execucao do bubbleSort: %.20fs\n", (float)(end - begin) / CLOCKS_PER_SEC);

  begin = clock();
  qsort(nums2, n, sizeof(float), cmp);
  end = clock();
  printf("tempo de execucao do quickSort: %.20fs", (float)(end - begin) / CLOCKS_PER_SEC);
